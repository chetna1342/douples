...
# Load your data
with open('pcc.json') as json_file:
...
# Train the model
model.fit(np.array(X_train), np.array(y_train), epochs=200, batch_size=16, validation_data=(np.array(X_test), np.array(y_test)), verbose=1)
...
model.save('LSTM_Model_PCC.keras')
pickle.dump(label_encoder, open('label_encoder_pcc.pkl', 'wb'))
